...
import { buildRssUrl, fetchJobs, type DjinniJob } from './rss';
...
class DjinniBotApp {
  ...
  private readonly formatter = new UiFormatter();
  ...
  public start(): void {
    ...
    this.bot.start({
      onStart: ({ username }) => {
        console.log(
          `Bot @${username} started. Poll interval: ${this.pollIntervalMs}ms`,
        );
      },
    });
  }
  ...
  private registerHandlers(): void {
    this.bot.command('start', async (ctx) => {
      ...
      if (!this.hasFullSettings(chatId)) {
        await ctx.reply(
          'Привіт! Я парсю Djinni RSS і надсилаю нові вакансії. Почнемо з вибору категорії.',
          {
    ...
    this.bot.callbackQuery('poll:now', async (ctx) => {
      ...
      try {
        ...
        await ctx.reply(
          sentCount > 0
            ? `Надіслано вакансій: ${sentCount}`
            : 'Нових вакансій поки немає.',
        );
      } catch {
    ...
    this.bot.callbackQuery('cat:set_all', async (ctx) => {
      ...
      saveCategory(chatId, ALL_CATEGORIES_VALUE);
      await ctx.answerCallbackQuery({
        text: `Категорія: ${ALL_CATEGORIES_LABEL}`,
      });
    ...
    this.bot.callbackQuery('exp:open', async (ctx) => {
      ...
      ensureUser(chatId);
      this.expDrafts.set(chatId, new Set(getUser(chatId)?.expLevels ?? []));
    ...
    this.bot.callbackQuery('exp:save', async (ctx) => {
      ...
      await ctx.answerCallbackQuery({
        text:
          draft.length > 0
            ? 'Налаштування досвіду збережено'
            : 'Фільтр досвіду вимкнено',
      });
      await this.renderSettings(ctx);
    });
  ...
  private registerCommands(): void {
    this.bot.api.setMyCommands([
      ...
      { command: 'settings', description: 'Змінити категорію/досвід' },
  ...
  private registerErrorHandler(): void {
    this.bot.catch((err) => {
      ...
      if (e instanceof GrammyError) {
        console.error('Grammy error:', e.description, e.error_code);
        return;
      }
  ...
  private buildCategoryKeyboard(page: number, chatId: number): InlineKeyboard {
    const totalPages = Math.ceil(DJINNI_CATEGORIES.length / CATEGORY_PAGE_SIZE);
    const clampedPage = Math.min(
      Math.max(page, 0),
      Math.max(totalPages - 1, 0),
    );
    const selectedCategory = getUser(chatId)?.category;

    const start = clampedPage * CATEGORY_PAGE_SIZE;
  ...
  private async primeCurrentFeed(chatId: number): Promise<void> {
    ...
    try {
      ...
      const rssUrl = buildRssUrl(rssCategory, user.expLevels);
      const jobs = await fetchJobs(rssCategory, user.expLevels);
      const latestLink = this.getLatestLink(jobs);

      if (latestLink) {
        setLastJobLink(chatId, latestLink);
      }
      ...
      console.error(`Prime feed failed for chat ${chatId}`, error);
    }
  }
  ...
  private async processUserFeed(chatId: number): Promise<number> {
    ...
    if (!latestLink) {
      this.logInfo(
        `poll chat=${chatId} url=${rssUrl} jobs=0 latest_link=none sent=0`,
      );
      return 0;
    }

    if (user.lastJobLink === null) {
      setLastJobLink(chatId, latestLink);
      this.logInfo(
    ...
    const anchorIndex = jobs.findIndex((job) => job.link === user.lastJobLink);
    if (anchorIndex === 0) {
      this.logInfo(
        `poll chat=${chatId} url=${rssUrl} jobs=${jobs.length} previous_link=${user.lastJobLink} latest_link=${latestLink} sent=0`,
      );
      return 0;
    }
    ...
    if (anchorIndex === -1) {
      setLastJobLink(chatId, latestLink);
      this.logInfo(
        `poll chat=${chatId} url=${rssUrl} anchor_missing previous_link=${user.lastJobLink} latest_link=${latestLink} jobs=${jobs.length} sent=0`,
      );
      return 0;
    }
    ...
    for (const job of newJobs) {
      try {
        await this.bot.api.sendMessage(
          chatId,
          this.formatter.formatJobMessage(job),
          {
            parse_mode: 'HTML',
    ...
    setLastJobLink(chatId, latestLink);
    this.logInfo(
      `poll chat=${chatId} url=${rssUrl} jobs=${jobs.length} previous_link=${user.lastJobLink} latest_link=${latestLink} new_jobs=${newJobs.length} sent=${sent}`,
    );
  ...
  private async pollAllUsers(): Promise<void> {
    if (this.pollIsRunning) {
      this.logInfo('poll tick skipped: previous cycle is still running');
      return;
    }
    ...
    try {
      ...
      this.logInfo(`poll tick started: users=${users.length}`);

      for (const user of users) {
        try {
          await this.processUserFeed(user.chatId);
        } catch (error) {
          console.error(`Polling failed for chat ${user.chatId}`, error);
        }
      }

      this.logInfo('poll tick finished');
    } finally {
...
new DjinniBotApp(bot, POLL_INTERVAL_MS).start();
